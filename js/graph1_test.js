var dashGraph_test = dashGraph_test || {};
dashGraph_test.gadget = gadget;


 

dashGraph_test.onLoad = function(loadedGadgets,gadget) {
	var comma0 = "[<0]#,#;[>0]#,#;-";

	var chart;
	
	requireScript('numeral', 0, dsg_site_root + 'application/third_party/DSG/numeral.js', function(){ 
		requireScript('anychart', 0, dsg_site_root + 'application/third_party/AnyChart/anychart-bundle.min.js', function(){ 
			chart = anychart.column();
		
			var wId = "gadget_" + loadedGadgets.dashGraph_test.gadgetId;
			var jId = '#' + wId;
			$(jId).empty(); 
