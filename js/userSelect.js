...
userSelect.onLoad = function(loadedGadgets,gadget) {
	
    // userSelect.gadget = gadget;
    // var wId = "gadget_" + loadedGadgets.userSelect	.gadgetId;
	// var jId = '#' + wId;
	...
	// $.get(dsg_site_root + "admin/typeaheadUserData/" + "?api=json", function(data) {
		...
		// for(var i=0; i<JSONData.length;i++) {
			// idList[i] = JSONData[i].Name + ' - ' + JSONData[i].User_ID.toString();
			// JSONData[i].combine = idList[i];
			// refArray[idList[i]] = JSONData[i].User_ID;
		// }
		// // dLog(refArray);
