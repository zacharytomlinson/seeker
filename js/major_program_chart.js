var major_program_chart = major_program_chart || {};

major_program_chart.onLoad = function(loadedGadgets, gadget) {
	major_program_chart.refresh(loadedGadgets, gadget);	
}

major_program_chart.refresh = function(loadedGadgets, gadget) {
	dLog('This is reloading');
	major_program_chart.gadget = gadget;
	var comma0 = "[<0]#,#;[>0]#,#;-";

	var chart; 
	chart = anychart.column();

	var wId = "gadget_" + loadedGadgets.major_program_chart.gadgetId;
	var jId = '#' + wId;
	$(jId).empty(); 
	...
	$.get(dsg_site_root + "majorProgramCenter/mpGraphData/" + gadget.params[0], function(data) {
		// dLog("retrieved data");
