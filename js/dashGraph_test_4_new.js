var dashGraph_test_4_new = dashGraph_test_4_new || {};

dashGraph_test_4_new.onLoad = function(loadedGadgets,gadget) {
	
	var comma0 = "[<0]#,#;[>0]#,#;-";

	dashGraph_test_4_new.gadget = gadget;
	var chart; 
	
	chart = anychart.column();

	var wId = "gadget_" + loadedGadgets.dashGraph_test_4_new.gadgetId;
	var jId = '#' + wId;
	$(jId).empty(); 
	$(jId).append('<div id="graphspinner"><center><img src="' + dsg_site_root + 'theme/img/ajax-loader-large.gif" border=0></center></div>');
	
	
	var gadgetData = (loadedGadgets.dashGraph_test_4_new);
