var graph1 = graph1 || {};
//graph1.gadget = gadget;
graph1.first = true;

graph1.onLoad = function(loadedGadgets, gadget) {
    graph1.gadget = gadget;
	
	var wId = "gadget_" + loadedGadgets.graph1.gadgetId;
	var jId = '#' + wId;
	...
	if(graph1.first){
		graph1.first = false;
		graph1.refreshGraph(loadedGadgets,gadget);
	} else {
		// dLog('reload ajax?');
		graph1.spreadRefresh();
	}
}
...
graph1.spreadRefresh = function() {
	...
	$.get(dsg_site_root + "api/getFinancialGraphData/" + graph1.gadget.params[0], function(data) {
...
graph1.refreshGraph = function(loadedGadgets,gadget) {
	var wId = "gadget_" + loadedGadgets.graph1.gadgetId;
	var jId = '#' + wId;
	var comma0 = "[<0]#,#;[>0]#,#;-";
